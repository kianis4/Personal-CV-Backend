        ...
        if (item.media_type === 'CAROUSEL_ALBUM') {
            ...
            for (const obj of children) {
                ...
                if (obj.media_type === 'IMAGE') {
                    ...
                    if (counter == 1){
                        post.images.push({ name: outputFilenameChild, status: 'current', imageSrc: `http://localhost:3000/image/${outputFilenameChild}` });
                    }else{
                        post.images.push({ name: outputFilenameChild, status: '', imageSrc: `http://localhost:3000/image/${outputFilenameChild}` });
                    }
